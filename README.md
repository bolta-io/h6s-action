...
AI 에이전트용 한국 금융 데이터 API를 GitHub Actions 워크플로우에서 한 step으로 호출합니다. 홈택스·은행 입출금내역과 카드 거래내역을 CSV(기본) 또는 JSON/JSONL/Markdown/YAML 파일로 저장하고, 다음 step에서 `outputs.path`로 받아 PR·Artifact·DB 적재·알림 등 원하는 후속 액션에 연결할 수 있습니다.
...
GitHub Actions에는 `H6S_API_KEY` secret 1개를 등록합니다. 콘솔(https://h6s.ai)에서 API Key를 발급한 뒤 repository secret `H6S_API_KEY`에 등록하고, 위 예시처럼 `${{ secrets.H6S_API_KEY }}`를 전달합니다. 수집하려는 provider의 자격증명도 같은 워크스페이스에 미리 등록해야 합니다. 매칭되는 자격증명이 없으면 `CREDENTIAL_INSUFFICIENT_FOR_PROVIDER`로 실패합니다.

## 무엇을 자동화할 수 있나

### 매월/매주 입출금내역을 repo 에 자동 PR
은행 입출금내역을 정기적으로 수집해 plain-text accounting (beancount/hledger) 저장소에 PR로 올립니다.
...
은행 입출금내역 + 매출/매입 세금계산서 + 매출/매입 현금영수증을 matrix로 병렬 수집 → 한 PR.
...
### 여러 은행 계좌를 병렬로 수집
본 계좌 + 분리 계좌(다른 기관)를 `matrix.provider`로 한 번에 수집합니다. provider만 주면 콘솔에 등록된 credential 중 매칭되는 항목이 자동 선택됩니다.
- 예: `CB_IBK` + `CB_KB` + `CB_SHINHAN`
- 예제: [multi-provider-matrix.yml](./examples/multi-provider-matrix.yml)

### 법인카드 승인내역을 정기 수집 — 지출 모니터링
법인카드 승인 시점 거래(매입 처리 전 단계 포함)를 주기적으로 수집해 지출을 추적합니다.
...
`outputs.summary`와 `outputs.count`를 메시지에 넣어 성공/실패를 한 줄로 보고합니다.
...
Claude Code Action이 수집된 CSV를 읽고 beancount 저널에 전표를 추가합니다. 대차 정합성 검증까지 통과한 PR을 생성합니다.
- 예제: [ai-ledger-trigger.yml](./examples/ai-ledger-trigger.yml)

## 스펙 카탈로그 / LLM 가이드

`schema` / `provider` / `params`에 어떤 값을 넣어야 하는지는 한 곳에서 확인합니다. 사람도, AI 어시스턴트(Claude Code/Cursor)도 동일한 진입점을 사용합니다.
...
> AI 에이전트가 워크플로우를 자동 생성할 때는 `https://h6s.ai/llms-full.txt` 한 URL만 fetch하면 됩니다. schema id, request 필드(필수/선택/타입), 에러 코드까지 한 번에 들어오고, 아래 `params` 입력값(JSON)도 거기서 그대로 매핑할 수 있습니다.
...
| `cli-version` | | (액션 동기) | 액션 태그는 `@v2` (rolling 메이저) / `@v2.5.4` (정확한 버전 핀). 이 입력으로 특정 cli 버전을 강제할 수 있습니다 |
| `timeout` | | `35m` | data-job 폴링 한도. API 응답의 `maxWaitSeconds` 보다 여유 있게 잡는 것을 권장 |
...
| `count` | 정수 | 레코드 수 |
| `job-id` | UUID | data-job 식별자. 콘솔에서 같은 수집 요청 확인 가능 |
