...
### 매월/매주 거래내역을 repo 에 자동 PR
은행 거래내역을 정기적으로 수집해 plain-text accounting (beancount/hledger) 저장소에 PR 로 올린다.
- schema: `bank.transactions.cb.v1` · provider: `ibk_bank` 등
- 결과: PR → `data/bank/*.csv`
- 예제: [monthly-bank-collect.yml](./examples/monthly-bank-collect.yml)

### 한 워크플로우에서 은행 + 홈택스 다종 수집
은행 거래내역 + 매출/매입 세금계산서 + 매출/매입 현금영수증을 matrix 로 병렬 수집 → 한 PR.
...
- 예: `ibk_bank` + `kb_bank` + `shinhan_bank`
...
- **provider**: `ibk_bank` (기업), `kb_bank` (국민), `shinhan_bank` (신한), `hometax` (홈택스)
- **schema**: `bank.transactions.cb.v1` (은행 입출금내역), `hometax.tax-invoices.{sales,purchase}.v1` (세금계산서 매출/매입), `hometax.cash-receipts.{sales,purchase}.v1` (현금영수증 매출/매입)
...
| `provider` | ✓ | — | providerCode (예: `ibk_bank`, `kb_bank`, `hometax`). 콘솔에 등록된 credential 중 이 provider 에 매칭되는 1개가 자동 선택. 전체 목록: https://h6s.ai/llms-full.txt |
