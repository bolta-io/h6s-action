...
홈택스·은행 입출금내역과 카드 거래내역을 GitHub Actions 워크플로우에서 한 step으로 수집합니다. 결과는 CSV(기본) 또는 JSON/JSONL/Markdown/YAML로 파일에 저장되고, 다음 step에서 `outputs.path`로 받아 PR·Artifact·DB 적재·알림 등 원하는 후속 액션에 연결할 수 있습니다.
