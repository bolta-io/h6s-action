...
#   3) h6s data-jobs get <id> --wait → 폴링 + 최종 메타 (kind/recordCount) 캡처
...
: "${INPUT_PROVIDER:?provider 입력이 비어 있습니다 (예: CB_IBK, CB_KB, HOMETAX)}"

# stdin JSON 에서 필드 1개를 뽑는다. $1=json 문자열, $2=`o` 기준 JS 식 (미존재 시 빈 문자열).
json_field() {
  printf '%s' "$1" | node -e "
let d=''; process.stdin.on('data',c=>d+=c).on('end',()=>{
  try { const o = JSON.parse(d); process.stdout.write(String($2 ?? '')); } catch { process.exit(1); }
})"
}
...
echo "$JOB_JSON"
JOB_ID="$(json_field "$JOB_JSON" 'o.id')"
echo "::endgroup::"
...
KIND="$(json_field "$FINAL_JSON" 'o.kind')"
COUNT="$(json_field "$FINAL_JSON" 'o.recordCount ?? 0')"

if [ "$KIND" != "Succeeded" ]; then
  ERRCODE="$(json_field "$FINAL_JSON" 'o.error?.code')"
  echo "::error::data-job 실패 (kind=${KIND}${ERRCODE:+, error=${ERRCODE}})" >&2
  exit 1
fi
